...
/// └────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┴────┘
/// ```
pub const NW_MASK: Macrocell<Bool8x8> = Macrocell {
...
const fn stamp_nw(cell: Macrocell<Leaf>) -> Leaf {
    Leaf::dead()
}
...
pub const fn evolve(cell: Macrocell<Bool8x8>, rule: Rule) -> Bool8x8 {
    let a = cell.nw.and(NW_MASK.nw).up(2).left(2);
    let b = cell.ne.and(NW_MASK.ne).up(2).right(6);
    let c = cell.sw.and(NW_MASK.sw).down(6).left(2);
    let d = cell.se.and(NW_MASK.se).down(6).right(6);
    let abcd = Bool8x8::FALSE.or(a).or(b).or(c).or(d);
